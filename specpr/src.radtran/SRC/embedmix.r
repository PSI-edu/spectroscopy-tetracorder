...
#     Further, it computes the reflectance assuming the
#     materials are mebedded in another matrix, is in a rock
#     or oil emulsion.
#
...
5     format (/, 9x, 'REFLECTANCE COMPUTATIONS ',
		'for INTIMATE, MOLECULAR, EMBEDDED and AREAL MIXTURES', //,
...
/,'           (you give component abundances and partcle sizes).',
/)
...
#
#  Read file for input comparison method, method 2  (note, at present, the
#          above code will not allow this mode as more programming is needed
#
...
63		format (1x, i6, ' optical index set allowed',
			' in this computation',/)
	} else {            # mode 1: molti-componenet mixture
		call getnst2(il)
		if (il == ihe || il == ihx) go to 10000
	}
...
123					format ('ch=',i3,'  wave=',f12.6,'  nh=',f12.6,
		...
		if ( matrixptr == iminr ) {  # this component is the matrix material

			write (ttyout,"(' NOTE: material',i5,' is the matrix marerial')") matrixptr

			do j=1, nchans {

				xemn(j)  = xn(matrixptr,j)    # index of refraction
				xemk(j)  = xk(matrixptr,j)    # absorption coefficient 0n inverse cm
				xemkm(j) = xkm(matrixptr,j)   # index of refraction imaginary part
...
430	format (' Type  nf  for no first-surface reflection',/,
	...
	if ( iopcon(i-1:i) == 'nf' ) {
		...
		do i = 1,nchans {                # subtract first surface reflection
			r(i) = r(i) - rfirst(i)
		}
