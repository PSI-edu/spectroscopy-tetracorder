...
#ccc                    = 0 no error
#ccc                    = 1 error
#ccc                    = 12 if when enter bdmset, ier=12, then
#ccc                         no error messages issued, but warnings
#ccc                         in tetracorder, the feature will be disabled
#ccc
...
#ccc
	...
	minch=0
	maxch=0
	imod12 = 0 # default, if input ier =12 will go into warning mode for tetracorder

#
# check input parameters are correct
#
	if (ier == 12) {
		imod12 = 12 # input ier =12 so go into warning mode for tetracorder
	}
	ier = 0  # reset ier for errors
	if (cl1 > cl2) {
		write (ttyout, 100)
...
# first left continuum average:
	...
	if (n < 1) {
	   if (imod12 == 12) {
		write (ttyout,119)
119		format (9x,'WARNING: all points in reference spectrum,',
