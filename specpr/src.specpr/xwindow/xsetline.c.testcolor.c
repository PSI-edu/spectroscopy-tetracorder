...
int *i;
{
	...
	int dash_offset = 0;
	char *colorname = "red";  /* test for color */
	Colormap cmap; 
	XColor color, ignore; 
	...
	cmap = DefaultColormap (SDisplay, screen);
	XAllocNamedColor(SDisplay, cmap, colorname, &color, &ignore);
	fgcolor = color.pixel;
	gcval.foreground = fgcolor;
	gcval.background = bgcolor;

	XSetForeground(SDisplay, SGC, redColor);
}
