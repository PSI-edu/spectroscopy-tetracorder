...
46	call crtin
	i = 1
	call wjfren (i,x,il)
	if (i >= 80 & il == 0) go to 44     # blank line, read again
	if (i > 1) i = i-1
	jend=lnb(iopcon)
	length = jend - i + 1
	lengthhist = length
...
146	call crtin
	i = 1
	call wjfren (i,x,il)
	if (i >= 80 & il == 0) go to 144    # blank line, read again
	if (i > 1) i = i-1
	jend=lnb(iopcon)
	length = jend - i + 1
	if (length > 80) length = 80
...
140	call crtin
		# write(ttyout,*) ' debug1a: iopcon: ',iopcon
	i=1
150	call wjfren (i,a,il)
	if (i >= 80 & il == 0) {
		# write(ttyout,*) ' debug1b: iopcon: ',iopcon
		go to 50     # blank line, read again
	}
	if (iwidok(il) == 1) {
		iwtmpf = il     # wavelength ID
		call wjfren (i,b,ik)
		if (b<=0 || b>maxrec) {
			call what (i)
			write(ttyout,165)
...
210	call crtin
		# write(ttyout,*) ' debug temp1a: iopcon: ',iopcon
	i=1
220	call wjfren (i,x,il)
	if (i >= 80 & il == 0) {
		# write(ttyout,*) ' debug temp1b: iopcon: ',iopcon
		go to 200    # blank line, read again
	}
	if (il == iht) {
		i=i-1
	...
	write (lunhist,*) ' temperature ', dtemperature(1), dtemperature(2), ' K'
...
420	call wjfren (i,x,il)
	if (i >= 80 & il == 0) go to 400    # blank line, read again
	if (il == ihp) {
		i=i-1
	...
	write (lunhist,*) ' pressure ', dpressure(1), dpressure(2), ' bar'

##################### Geologic origins ###################################

500	if (cmdverbose(-1) <= 1) write (ttyout, 401)
501	format (1x, 'Type:  geology  to calculate geologis origins',/,
		1x, '    or nogeology to skip ',/)

510	call crtin
	i=1
520	call wjfren (i,x,il)
	if (i >= 80 & il == 0) go to 500    # blank line, read again
	if (il == ihg) {
		i=i-1
		if ( iopcon(i:i+6) == 'geology' ) {

			geoflag = 1      # = 1 include geologic origins

			write (ttyout,*) '  Include geologic origins calculations'
		} 

	} else  if (il == ihn) {
		i=i-1
		if ( iopcon(i:i+8) == 'nogeology' ) {

			geoflag = 0
			write (ttyout,*) '  NOT including geologic origins calculations'
		}
		...
		write (ttyout,*) '  ERROR: expecting geology or nogeology but found ', iopcon(1:30)
		go to 500
	}
	if (geoflag == 1) {
		write (lunhist,*) ' geology   Include geologic origins calculations'
	} else {
		write (lunhist,*) ' nogeology   OT including geologic origins calculations'
	}
	write (ttyout, *) ' Reading goelogy files'
	call readgeo
...
620	call wjfren (i,x,il)
	if (i >= 80 & il == 0) go to 600    # blank line, read again
	if (il == ihm) {
		i=i-1
