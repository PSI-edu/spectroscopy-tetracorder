...
# Roger N. Clark, Planetary Science Institute, PSI
	...
	integer*4	lunegeocube
	integer*4	lunengeotbl

        integer*4       lungeotable  # lun for reading  geologic-origins/geological-origin-table-by-material.txt
        integer*4       lunmatclass  # lun for reading  geologic-origins/material-classification.txt
        integer*4       lungmattble  # lun for writing info/geologicmaterialstable.txt
        integer*4       lungeocube   # lun for geologic origins cube (2026: 12 channels per pixel)
        ...
        parameter       (maxmatcomps=8)  # maximum number materials per reference spectrum
...
###          is very near the limit.  When maxmat needs to go above 670, 
...
################################################################################
	...
	parameter	(luntr=59)    # lun for tetracorder-format file
        ...
        parameter       (lungeotable=72)  # lun for reading  geologic-origins/geological-origin-table-by-material.txt

	parameter	(lunmatclass=73)  # lun for reading  geologic-origins/material-classification.txt

	parameter	(lungmattble=74)  # lun for writing info/geologicmaterialstable.txt
					  #         list for each component in each reference spectrum

	parameter       (lungeocube=75)  # lun for geologic origins cube (2026: 12 channels per pixel)

	parameter	(geochans=11)    # number of channels in the geologic origins cube
...
#					  #     for each pixel in the line.
	...
	integer*4    maskflag		# = 1 include a mask image to skip analyses at a pixel
					# = 0 no masking
...
#                 removed shapes.  2 values are used for fuzzy logic.
	...
	real*4 dtemperature(2)        # data set temperature range
        ...
        integer*4 refdata(maxmat)       # what the reference data are:
					# = 0 raw (no change to input
					# = 1 radiance
					# = 2 reflectance
					# = 3 emittance
	...
	common /geomap/ geoflag
	common /geomap/ gmaterials, geoorigin, matname, matclassname, matgroupname
	common /geomap/ ngmaterials, nmatnames
	common /geomap/ nmatcomponent, matcomponent, matcomponidx, matcompgeorg
	common /geomap/ matcompclass, matcomponentabundance
        common /geomap/ matidconfidence, matidclassconfidence

	integer*4    geoflag	#  0 = no geologic origins
                                #  1 = geologic origins enabled by user input
                                #  2 = geologic origins files read
                                #  3 = 2 + geologic origins cube opened

	# geoloigic origins table from:   geologic-origins/geological-origin-table-by-material.txt 
	#                                note: actual number of ngmaterials will be way less than maxmat
	#                   the table is read by readgeo.r
	#
	# Geologic origins starts in the expert system (cmd.lib.setup.*) where the components in
	#                  each reference spectrum is defined in the "list materials" section.
	#                  example: "Fe3+ Hematite.10+Goethite.90 hg1" has components:`
	#                            hematite_smallgrain  and
	#                            goethite
	#                  the component names for each reference is in: "matcomponent "
	#
	#    The components are listed in file  geologic-origins/geological-origin-table-by-material.txt
	#    That file is read into the array: names into       variable   "gmaterials"
	#                                      geo origins into variable   "geoorigin"
	#                                      number of materials =       "ngmaterials"
	#
	#    In the "Fe3+ Hematite.10+Goethite.90 hg1" example, there  2 components
	#           assuming "hematite_smallgrain" has  index=   105 in geologic-origins/geological-origin-table-by-material.txt
	#              and   "goethite"            has  index=    98 then
	#           
	#             matcompgeorg(1,imat) = 105   # hematite_smallgrain
	#             matcompgeorg(2,imat) =  98   # goethite
	#
	#             Now any material component geo origin can be quickly referenced with
	#                  geoorigin(i, matcompgeorg(1,imat))  i = 1 to geochans   # hematite_smallgrain
	#                  geoorigin(i, matcompgeorg(2,imat))  i = 1 to geochans   # goethite
	#
	#    in the code, a material is referenced by the variable "imat" ranging from 1 to "maxmat"
	#
	#    To speed lookups, pointers to the tables are built
	#      "matcomponidx(maxmatcomps,maxmat)" is the index pointer for each material 
	#      "matcompgeorg(maxmatcomps,maxmat)" is the index pointer for each material in the geologic origins table
	#
	#    Given "matcompgeorg(iatcomp,imat)" the geologic origin is:
	#                                       iorig = matcompgeorg(iatcomp,imat)
	#                                    then origin is "geoorigin(igeochans,iorig)"
	#                                    where igeochans goes from 1 to geochans
	#
	#
	#
	character*32 gmaterials(maxmat)
	real*4       geoorigin(geochans,maxmat)   # geologic origins for each material component
	integer*4    ngmaterials                  # number of gmaterials

	# material classifications from:   geologic-origins/material-classification.txt
	#                                note: actual number of nmatnames will be way less than maxmat
	character*32 matname(maxmat)
	character*32 matclassname(maxmat)
	character*32 matgroupname(maxmat)
	integer*4    nmatnames              # number of matnames

	# materials in each reference spectrum:
	#
	character*32 matcomponent(maxmatcomps,maxmat)          # material component name  (chars should be divisible by 4)
	integer*4    matcomponidx(maxmatcomps,maxmat)          # material component index to the name
	integer*4    matcompgeorg(maxmatcomps,maxmat)          # material component index to the geologic origin
	character*32 matcompclass(maxmatcomps,maxmat)          # material component class (chars should be divisible by 4)
	integer*4    nmatcomponent(maxmat)                     # numner of material components per reference spectrum
	real*4       matcomponentabundance(maxmatcomps,maxmat) # material component abundance

	real*4       matidconfidence(maxmat)       # material component ID confidence (0 - 1.0)
	real*4       matidclassconfidence(maxmat)  # material component ID class confidence (0 - 1.0)
                                                   # confidence:  (use real numbers in calculations, integers below for history)
                                                   #  expert system:   0 - 9  scale but internally 0 to 1.0
	integer*4 idmatconfidence(2,maxmat)  # confidence number input in expert system real numbers as above
                                           #    note: 1 is a shape match, so doesn't help ID confidence
                                           #     note:  matidconfidence(maxmat)      = (idmatconfidence(1,imat)-1) /8.0
                                           #     note:  matidclassconfidence(maxmat) = (idmatconfidence(2,imat)-1) /8.0
                                           # 0 - 9 (output 8-bit image scaled to 0 - 255)
                                           # 0 = no confidence
                                           # 1 = shape match only, including position
                                           # 2 - 9 = increasingly diagnostic.
                                           # actual pixel confidence = f*d * (idmatconfidence-1)/8
	...
	common /iostuff/ ndevurat,nrecurat,ndevrrat,nrecrrat
        ...
        character*40 mtitle(maxlibs,maxmat)      # title for each material (input library spectra)
