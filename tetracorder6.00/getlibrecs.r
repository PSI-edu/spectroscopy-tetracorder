...
70      format (///,40('-'),/,
			' Enter:',/,
			' Alternate Library letter (a, b, c, ...)  Name: ',
...
33		format (' ERROR: library letter code out of range', a1)
	...
	if (i >= 79 && il == 0) {
		call what(i)
		write (ttyout,*) 'ERROR: no alternate library name found, wanted:', altuse
		go to 69
	}
	...
	if (iflgalt == 0) { # invalid keyword
		...
		write (ttyout,*) '    Expected one of these alternates:'
		do iitmp = 1, numaltlib {

			write (ttyout,*) '   alternate library keyword: ', altlib(iitmp)
		}
		go to 69
	}
